        ...
        private static void DepositMoney(BankContext context, string username)
        {
            ...
            else
            {
                ...
                if (double.TryParse(MenuFunctions.CursorReadLine(), out double depositAmount))
                {
                    ...
                    MenuFunctions.footer();
                    Console.WriteLine($"\t\tDepositing {Math.Round(depositAmount, 2)} {selectedAccount.Currency} into {selectedAccount.Name}");

                    selectedAccount.Balance += depositAmount;
                    context.SaveChanges();
                    Console.WriteLine($"\t\tNew balance: {Math.Round(selectedAccount.Balance, 2)} {selectedAccount.Currency}");
                }
                else
                {
